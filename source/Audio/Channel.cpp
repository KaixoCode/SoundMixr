...
namespace Audio
{
	...
	void Channel::Level(float s, int c)
	{
		if (lines == 0)
			return;
	...
	void Channel::Process()
	{
		...
		// Input
		if (type & Channel::Type::Input || type & Channel::Type::Forward)
		{
		...
		if (type & Channel::Type::Output || type & Channel::Type::Forward)
		{
			float _avg = 0;
	...
	void Channel::Connect(const Pointer<Channel>& c)
	{
	...
	void Channel::Disconnect(const Pointer<Channel>& c)
	{
	...
	bool Channel::Connected(const Pointer<Channel>& c) const
	{
	...
	Channel::operator nlohmann::json()
	{
		nlohmann::json _json;
		_json["id"] = id;
		_json["type"] = type;
	...
	void Channel::operator=(const nlohmann::json& json)
	{
	...
	{
		nlohmann::json _json = Channel::operator nlohmann::json();
	...
	{
		Channel::operator=(json);
		for (auto& i : json["endpoints"])
			Add(i);
	}

	EndpointChannel::EndpointChannel(bool input)
		: Channel(Type::Endpoint | (input ? Type::Input : Type::Output))
	{}

	void EndpointChannel::NextCycle()
	{
		if (type & Channel::Type::Output)
			for (int i = 0; i < lines; i++)
